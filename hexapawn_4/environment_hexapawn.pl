...
is_board(B):- functor(B,b,16).

initial_board(b(b,b,b,b,e,e,e,e,e,e,e,e,w,w,w,w)) :- !.
...
make_move(LP,s(M,_,B1),s(M1,_,B2),none) :-
    \+(LP = human),
...
play(Strategy1,Strategy2,Game,Outcome,[Bead|Beads]) :-
    ...
    ((board(B2),drawn(s(M1,_,B2))) -> ( %write('END, draw'), nl,
                append([B2],[],Game),
                 playing(P3),
                    copy(Beads,[]),
